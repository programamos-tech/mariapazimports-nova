...
import type { StoreCartDrawerItem } from "@/components/store/StoreCartDrawerProvider";
...
export type StoreCartDrawerPanelProps = {
              ...
              <ul className="divide-y divide-stone-200/90 pb-2">
                {items.map((item) => (
                  <DrawerLine
                    ...
                    pending={linePending}
                    onAdjustQty={(next) =>
                      onAdjustQty(item.productId, item.variantId, next)
                    }
                  />
                ))}
              </ul>
