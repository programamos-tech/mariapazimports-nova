...
import { storeAuthFormSecondaryBtnClass } from "@/components/store/store-auth-form-primitives";
...
export function StoreLoginModal({ open, onClose, onOpenRegister }: Props) {
  const titleId = useId();
        ...
        <div className="store-cart-drawer-body-scroll flex min-h-0 flex-1 flex-col px-6 pb-8 pt-6 sm:px-8">
          ...
          <div className="mt-8">
            <Suspense fallback={<p className="text-sm text-stone-500">Cargando…</p>}>
              <StoreLoginForm onLoggedIn={onClose} />
            </Suspense>
          </div>
          ...
          <div className="mt-8">
            <button
              type="button"
              onClick={dismiss}
              className={storeAuthFormSecondaryBtnClass}
            >
              Volver a la tienda
            </button>
          </div>
