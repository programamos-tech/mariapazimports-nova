...
const sectionTitle =
  "px-3 py-2 text-[10px] font-semibold uppercase tracking-[0.2em] text-stone-500";
...
const liquidPanelClass =
  "absolute left-0 right-0 top-[calc(100%+8px)] z-[100] max-h-[min(65vh,440px)] overflow-y-auto overscroll-contain rounded-2xl border border-stone-200/95 bg-gradient-to-b from-white via-stone-50/98 to-stone-100/95 shadow-[0_20px_50px_-18px_rgba(28,25,23,0.18),inset_0_1px_0_0_rgba(255,255,255,0.9)] backdrop-blur-2xl backdrop-saturate-150";
  ...
  return (
    <div ref={rootRef} className="relative w-full min-w-0 overflow-visible">
      ...
      <div className="relative min-w-0" role="search">
        <IconSearch className="pointer-events-none absolute left-3.5 top-1/2 size-[18px] -translate-y-1/2 text-stone-400" />
        <input
          ref={inputRef}
          ...
          className="box-border min-w-0 w-full rounded-full border border-stone-200 bg-white py-2.5 pl-11 pr-10 text-sm text-stone-900 shadow-[0_1px_3px_0_rgb(28_25_23/0.06)] placeholder:text-stone-400 focus:border-stone-400 focus:outline-none focus:ring-2 focus:ring-stone-900/10"
        />
        {q.trim() ? (
          <button
            type="button"
            className="absolute right-2.5 top-1/2 -translate-y-1/2 rounded-full p-1 text-stone-500 transition hover:bg-stone-100 hover:text-stone-900"
        ...
        >
          ...
          {loading ? (
            <div className="flex justify-center py-8">
              <div className="size-7 animate-spin rounded-full border-2 border-stone-300/60 border-t-stone-700/80" />
            </div>
          ) : null}

          {showEmpty ? (
            <p className="px-4 py-8 text-center text-sm text-stone-700">
                    ...
                    {products.map((p) => (
                      <li key={p.id}>
                        <Link
                          ...
                          onClick={closePanel}
                          className="flex items-start justify-between gap-3 rounded-xl px-3 py-2.5 transition hover:bg-stone-200/70"
                        >
                          ...
                          <span className="shrink-0 tabular-nums rounded-full bg-stone-200/90 px-2 py-0.5 text-xs font-medium text-stone-800 ring-1 ring-stone-300/70">
                    ...
                    {customers.map((c) => (
                      <li key={c.id}>
                        <Link
                          ...
                          onClick={closePanel}
                          className="block rounded-xl px-3 py-2.5 transition hover:bg-stone-200/70"
                        >
                          ...
                          >
                            <div className="min-w-0">
                              <p className="flex flex-wrap items-center gap-2">
                                <span className="font-mono text-sm font-medium text-stone-950">
                                  #{o.invoiceRef}
                                </span>
                                <span
