...
export function ActivityLogCard({ row, actorDisplay, formatWhen }: Props) {
  ...
  return (
    <li className="rounded-lg border border-zinc-200/80 bg-white px-2.5 py-2 shadow-sm sm:px-3 sm:py-2.5">
      <div className="flex gap-2.5 sm:gap-3">
        ...
        <div className="min-w-0 flex-1">
          ...
          <p className="mt-0.5 text-sm font-semibold leading-snug text-zinc-900">{row.summary}</p>
          ...
          <p className="mt-1 flex flex-wrap items-center gap-x-1.5 text-[10px] text-zinc-500">
            <span className="font-medium text-zinc-600">{actorDisplay}</span>
            {shortId ? (
              <>
                <span className="text-zinc-300">·</span>
                <span className="font-mono text-zinc-500" title={row.entity_id ?? undefined}>
                  {shortId}
                </span>
              </>
            ) : null}
            {link ? (
              <>
                <span className="text-zinc-300">·</span>
                <Link href={link.href} className="font-medium text-blue-700 hover:underline">
                  {link.label}
                </Link>
              </>
            ) : null}
