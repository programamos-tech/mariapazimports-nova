...
export function VentasSalesTable({ rows }: { rows: VentaOrderRow[] }) {
  ...
  function rowKeyDown(e: React.KeyboardEvent, id: string) {
  ...
  const cardClass =
    "flex flex-col rounded-xl border border-zinc-200/90 bg-white p-3.5 shadow-sm ring-1 ring-zinc-950/5 transition hover:border-zinc-300 hover:bg-zinc-50/50 hover:shadow-md dark:border-zinc-700/90 dark:bg-zinc-900 dark:ring-white/[0.06] dark:hover:border-zinc-600 dark:hover:bg-zinc-800/40 dark:hover:shadow-lg sm:p-4";
          ...
          return (
            <li key={row.id} className="min-w-0">
              <Link
                ...
                className={`${cardClass} block outline-none focus-visible:ring-2 focus-visible:ring-zinc-400 focus-visible:ring-offset-2 dark:focus-visible:ring-zinc-500`}
              >
                <div className="flex items-start justify-between gap-2.5">
                  <div className="flex min-w-0 flex-1 items-start gap-2.5">
                    {fisica ? (
                      ...
                      <IconShoppingCart className="mt-0.5 size-[22px] shrink-0 text-indigo-600 dark:text-indigo-400" />
                    )}
            ...
            {rows.map((row) => {
              ...
              return (
                <tr
                  ...
                  role="link"
                  tabIndex={0}
                  aria-label={`Ver pedido ${ref} de ${row.customer_name}`}
                  onClick={() => openOrder(row.id)}
                  onKeyDown={(e) => rowKeyDown(e, row.id)}
                  className="cursor-pointer border-b border-zinc-100 bg-white transition hover:bg-zinc-50/80 focus-visible:bg-zinc-50/80 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-zinc-300 dark:border-zinc-800 dark:bg-zinc-900 dark:hover:bg-zinc-800/80 dark:focus-visible:bg-zinc-800/80 dark:focus-visible:ring-zinc-600"
                >
                  <td className="px-3 py-3.5 sm:px-4 md:px-5">
                    <div className="flex items-center gap-2.5">
                      {fisica ? (
                        ...
                        <IconShoppingCart className="size-5 shrink-0 text-indigo-600 dark:text-indigo-400" />
                      )}
