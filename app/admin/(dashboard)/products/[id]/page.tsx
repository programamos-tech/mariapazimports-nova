  ...
  return (
    <div className="mx-auto max-w-7xl space-y-6">
      <div className="rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-8 flex flex-wrap items-start gap-6 border-t border-zinc-100 pt-8">
          {img ? (
          ...
          <div className="grid min-w-0 flex-1 grid-cols-2 gap-4 sm:grid-cols-3 lg:grid-cols-6 lg:gap-6">
            ...
            <div>
              ...
              <p className="mt-1 text-lg font-bold tabular-nums text-zinc-900">{fmtUnits(stockTotal)}</p>
              <p className="mt-0.5 text-xs text-zinc-500">Local + bodega</p>
            </div>
            <div>
              <p className={labelClass}>Stock local</p>
              <p className="mt-1 text-lg font-bold tabular-nums text-zinc-900">{fmtUnits(stockL)}</p>
              <p className="mt-0.5 text-xs text-zinc-500">Punto de venta / mostrador</p>
            </div>
            <div>
              <p className={labelClass}>Stock bodega</p>
              <p className="mt-1 text-lg font-bold tabular-nums text-zinc-900">{fmtUnits(stockW)}</p>
              <p className="mt-0.5 text-xs text-zinc-500">Almacén de la sucursal</p>
            </div>
            <div>
              <p className={labelClass}>Stock reservado</p>
              <p className="mt-1 text-lg font-bold tabular-nums text-amber-600">{fmtUnits(reservedQty)}</p>
            ...
            <div>
              ...
              <p className="mt-1 text-lg font-bold tabular-nums text-zinc-900">{formatCop(cost)}</p>
            </div>
          </div>
        </div>
      </div>
      ...
      <section className="rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm sm:p-8">
        ...
        <div className="mt-6 grid gap-4 sm:grid-cols-3">
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/80 p-5">
            ...
            <p className="mt-2 text-xl font-bold tabular-nums text-zinc-900">{formatCop(plataStock)}</p>
            <p className="mt-1 text-xs text-zinc-500">
              Inversión en {fmtUnits(stockTotal)}
            </p>
          </div>
          <div className="rounded-xl border border-sky-100 bg-sky-50/90 p-5 ring-1 ring-sky-100/80">
            <p className={labelClass}>Margen bruto estimado</p>
            <p className="mt-2 text-xl font-bold tabular-nums text-zinc-900">{formatCop(margenBruto)}</p>
          ...
          <div className="rounded-xl border border-zinc-100 bg-zinc-50/80 p-5">
            ...
            <p className="mt-2 text-xl font-bold tabular-nums text-zinc-900">{margenPct}%</p>
        ...
        <section className="rounded-2xl border border-zinc-200/90 bg-white p-6 shadow-sm">
          ...
          <dl className="mt-4 space-y-3 text-sm">
            <div>
              ...
              <dd className="mt-0.5 font-mono font-medium text-zinc-900">{reference}</dd>
            </div>
            <div>
